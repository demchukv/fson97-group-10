...
</div>`;try{if(localStorage.getItem(s)){const a=JSON.parse(localStorage.getItem(s));Array.isArray(a)&&a.length===0?(console.log(""),t.favoritesContent.innerHTML=i):t.favoritesContent.innerHTML=`<ul class="favorites-group">${g(a)}</ul> 

    `,o=document.querySelectorAll(".favorites-delete")}else t.favoritesContent.innerHTML=i}catch(a){console.log(a)}o&&o.forEach(a=>{a.addEventListener("click",e=>{let f=localStorage.getItem(s),A=JSON.parse(f);const n=e.target.closest(".favorites-item"),p=n.dataset.id;localStorage.setItem(s,JSON.stringify(A.filter(({_id:d})=>d!==p)));const c=document.querySelector(".favorites-group");c.removeChild(n),c.children.length===0&&(t.favoritesContent.innerHTML=i)})});function g(a){return a.map(e=>`
